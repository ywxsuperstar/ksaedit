...
def parse_args():
    ...
    parser.add_argument(
        "--prompt",
        type=str,
        default="creat_data/eval-sdxl.csv",
    )
    parser.add_argument(
        "--checkpoints_sae",
        type=str,
...
def activation_modulation_across_prompt(blocks_to_save, steer_prompt, strength, steps, guidance_scale, seed):
    ...
    diff1 = cache['output'][blocks_to_save[0]][:,0,:]  # [1, 77, 768]
    diff2 = cache['output'][blocks_to_save[1]][:,0,:]  # [1, 77, 1280]
...
sae = SparseAutoencoder.load_from_disk(os.path.join(args.checkpoints_sae, 'final')).to(device, dtype=dtype) 

num_inference_steps = 50  # denoising steps  50
